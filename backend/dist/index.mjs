...
var usersModel = mongoose2.model(
  "userSchema",
  userSchema
);
...
var agentSchema = new mongoose3.Schema({
  firstName: {
    type: String
  },
  lastName: {
...
var userBookingModel = mongoose5.model(
  "userBookinSchema",
  userBookinSchema
);
...
var comparePass = (password, hashedPass) => __async(void 0, null, function* () {
  try {
    const match = yield bcrypt.compare(password, hashedPass);
    return match;
  } catch (error) {
...
var transactioSchema = new mongoose6.Schema(
  {
    userId: {
...
var transactionmodel = mongoose6.model(
  "transactionmodel",
  transactioSchema
);
  ...
  registerUser(req, res, next) {
    return __async(this, null, function* () {
      ...
      try {
        ...
        const password = yield bcrypt2.hash(userData.password, 10);
...
import { Schema as Schema4, model } from "mongoose";
var chatSchema = new Schema4(
  {
    chatName: {
      type: String
    },
...
import { Schema as Schema5, model as model2 } from "mongoose";
...
var messageModel = model2("Message", messageSchema);

// src/database/modules/controllers/repositories/chatRepository.ts
import { Error as Error2 } from "mongoose";
...
var userAuth = (req, res, next) => __async(void 0, null, function* () {
  ...
  if (authHeader && authHeader.startsWith("User-Bearer")) {
    ...
    try {
      const decoded = jwt2.verify(token, jwtSecretToken2, (err, decoded2) => {
        if (err) {
    ...
    return __async(this, null, function* () {
      try {
        ...
        if (!isValidPassword) {
          ...
          const token = jwt3.sign(info.email, "secretKey");
          const accesseduser = {
            agenttoken: token,
            userdetails: {
              id: info._id,
  ...
  registeragent(req, res, next) {
    return __async(this, null, function* () {
      try {
        ...
        const password = yield bcrypt3.hash(agentData.password, 10);
...
// src/database/modules/controllers/routes/agentRoute.ts
var agentRouter = express2.Router();
...
var adminSchema = new mongoose7.Schema({
  email: {
    type: String
  },
  password: {
...
var adminModel = mongoose7.model("adminSchema", adminSchema);
      ...
      try {
        if (loginresult[0]) {
          if (loginresult[0].password == userlogdata.password) {
            let admintoken = jwt4.sign(userlogdata.email, secret);
...
var adminAuth = (req, res, next) => __async(void 0, null, function* () {
  ...
  if (authHeader && authHeader.startsWith("Admin-Bearer")) {
    const token = authHeader.split(" ")[1];
    try {
      const decoded = jwt5.verify(token, secret2, (err, decoded2) => {
...
// src/database/modules/controllers/routes/adminRoute.ts
var adminRouter = express3.Router();
...
app.use("/image", express4.static(path.join(__dirname, "image")));
